...
class OrderField(models.PositiveIntegerField):
    description = "Oerdering field on a unique field"
    ...
    def _check_for_field_attribute(self, **kwargs):
        if self.unique_for_field is None:
            return [
                checks.Error("Order Filed must define a 'unique_for_field' attribute")
            ]       
        elif self.unique_for_field not in [f.name for f in self.model._meta.get_fields()]:
            return [
                    checks.Error("Order Filed entered does not match any model field")
                ]
